/**
 * From CodeWars -- https://www.codewars.com/kata/561e9c843a2ef5a40c0000a4
 *
 ...
 *
 * nil slice is expected when there are no gap between m and n. Example: gap(11,30000,100000) --> nil
 *
 * Ref https://en.wikipedia.org/wiki/Prime_gap
 */
function gap(desiredGap, startNumber, finalNumber) {
  let initialPrime = null;
  ...
  for (let i = startNumber; i <= finalNumber; i++) {
    ...
    if (i - initialPrime === desiredGap) {
      return [initialPrime, i];
    }
...
const isPrime = function() {
  ...
  return function checkPrime(number) {
    if (number in memo) {
      return memo[number];
    }
    ...
    //The only even number that is Prime is 2, which is checked in the previous condition
    if (number % 2 === 0) {
      memo[number] = false;
      return false;
    }

    //Check all numbers untill the square root of the number to see
    //if any of them is factorial
...
function isFactor(numberToCheck, goalNumber) {
  return (goalNumber/numberToCheck).toString().indexOf('.') === -1;
}
